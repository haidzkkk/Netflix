...
enum FeatureOfCategory{
  show, search
}

enum CategoryMovie{
  listCartoon(name: "Hoạt hình", slug: "hoat-hinh", path: AppConstants.KK_GET_CATEGORY_1, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  movieNew(name: "Phim mới nhất", slug: "phim-moi-cap-nhat", path: AppConstants.KK_GET_LIST_MOVIE, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  listMovieSingle(name: "Phim lẻ", slug: "phim-le", path: AppConstants.KK_GET_CATEGORY_1, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  listTvShow(name: "Tv show", slug: "tv-shows", path: AppConstants.KK_GET_CATEGORY_1, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  listSeries(name: "Phim bộ", slug: "phim-bo", path: AppConstants.KK_GET_CATEGORY_1, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  listMovieAction(name: "Hành động", slug: "hanh-dong", path: AppConstants.KK_GET_CATEGORY_2, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),
  listEmotional(name: "Tình cảm", slug: "tinh-cam", path: AppConstants.KK_GET_CATEGORY_2, feature: FeatureOfCategory.show, serverType: ServerType.kkPhim, ),

  kkSearch(name: "Tìm kiếm", slug: "tim-kiem", path: AppConstants.KK_GET_SEARCH, feature: FeatureOfCategory.search, serverType: ServerType.kkPhim, ),
  opSearch(name: "Tìm kiếm", slug: "tim-kiem", path: AppConstants.OP_GET_SEARCH, feature: FeatureOfCategory.search,  serverType: ServerType.oPhim, ),
  ;

  final String name;
  final String slug;
  final String path;
  final FeatureOfCategory feature;
  final ServerType serverType;
  const CategoryMovie({required this.name, required this.slug, required this.path, required this.feature, required this.serverType});

  static CategoryMovie? getCategoryMovie(String? slug){
    return CategoryMovie.valueCategoriesShow.firstWhereOrNull((category){
      return category.slug == slug;
    });
  }

  static List<CategoryMovie> get valueCategoriesShow {
    return CategoryMovie.values.where((category) => category.feature == FeatureOfCategory.show).toList();
  }

  static List<CategoryMovie> get valueCategoriesSearch {
    return CategoryMovie.values.where((category) => category.feature == FeatureOfCategory.search).toList();
  }

  static Map<String, CategoryMovie> get mapCategories {
    return Map.fromEntries(valueCategoriesShow.map((e) => MapEntry(e.slug, e)));
  }
